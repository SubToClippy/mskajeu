...
keyauthapp = api(
    name = "bot",
    ownerid = "kb7WZdRm9u",
    secret = "e661b3d8eb2090fc75cac32984bdb9868e4e67dcc4a5aa136d630baa262aa027",
    version = "1.0",
...
def validate():
    if keyauthapp.license(config["license_key"]):
        ...
        print(Fore.GREEN + "Successfully Logged Into License Enjoy gg/boostworld" + Fore.RESET)
        time.sleep(2)
...
activity = discord.Activity(type=discord.ActivityType.playing, name=".gg/boostworld")

bot = discord.Bot(command_prefix="*", activity=activity, status=discord.Status.dnd)

os.system(f"title fh Boost Bot")

@bot.command(name="help", description="Display help menu")
async def help_command(ctx):
    embed = discord.Embed(title="Help Menu", color=0x2E3135, 
        description="➜**/boost** | ضرب بوست لسيرفر \n➜**/cashapp** |اسم الكاش اب \n➜**/restock** | أعد تخزين التوكنات \n➜**/stock** | تقدر تشوف كم عندك توكنات وبوستات \n➜**/whitelist** | اضافه شخص لاستعمال البوت\n➜**/paypal** | رابط البايبال \n➜**/unwhitelist** | ازاله ادمن \n➜**/website** | رابط صفحه المتجر \n➜**/restart** | لاعاده تشغيل البوت (Iالاونر فقط)\n➜**/sorter** | يفرز التوكن ايمال:باسورد:توكن \n➜**/givetokens** | اعطاء شخص توكن شهر او 3 اشهر\n***➜/1Month or/3 month***| اسعار البوستات الشهر و 3 اشهر")
    embed.set_image(url="https://cdn.discordapp.com/icons/1072407607479894036/76119c775b6db07df2e51cc21f40ad54.png?size=1024")
...
async def website_command(ctx):
    embed = discord.Embed(title="fh boost", color=0x000000, 
        description="➜[Coming Soon](https://discord.gg/boostworld)")
    embed.set_image(url="https://images-ext-1.discordapp.net/external/n8HXUM8qQ2gIKms9gVhpcmFiWXLSXcWmYYUiOjtOVsY/%3Fsize%3D1024/https/cdn.discordapp.com/icons/1072407607479894036/76119c775b6db07df2e51cc21f40ad54.png")
    embed.set_footer(text="discord.gg/boostworld")
    return await ctx.respond(embed=embed)
    
    
@bot.slash_command(guild_ids=[config["guild_id"]], name="vouch", description="shows someone how to vouch")
async def on_message(message):
   embedVar = discord.Embed(title="Vouch Command", description=f"\nPlease Vouch in <#1077728418667450428>\n**Usage:** `+rep (@user) (reason)`")
   embedVar.set_thumbnail(url="https://cdn.discordapp.com/icons/1072407607479894036/76119c775b6db07df2e51cc21f40ad54.png?size=1024")
   await message.respond(embed=embedVar)

@bot.slash_command(guild_ids=[config["guild_id"]], name="3months", description="Shows FH BOOST Boost Prices for 3 months")
async def prices(ctx):
    embed=discord.Embed(title="**3 Month Bost Prices**", description=f"**Server Boosts ⋅ 3 Months\n\nLevel 1 Boosts ⋅ $1\nLevel 2 Boosts ⋅ $3.50\nLevel 3 Boosts ⋅ $6**\nBuy here <#1074096057161617449>", color=0x000000)
    embed.set_image(url='https://cdn.discordapp.com/icons/1072407607479894036/76119c775b6db07df2e51cc21f40ad54.png?size=1024')
    embed.set_footer(text="discord.gg/boostworld")
    await ctx.respond(embed=embed)

@bot.slash_command(guild_ids=[config["guild_id"]], name="1month", description="Shows FH BOOST Boost Prices")
async def prices(ctx):
...
async def cashapp_command(ctx):
    embed = discord.Embed(title="CashApp Payment", color=0x2E3135, 
        description="➜[$PandaBST](https://cash.app/$PandaBST)")
    embed.set_thumbnail(url="https://cdn.discordapp.com/icons/1072407607479894036/76119c775b6db07df2e51cc21f40ad54.png?size=1024")
    embed.set_footer(text="discord.gg/boostworld")
    return await ctx.respond(embed=embed)

@bot.command(name="paypal", description="Displays paypal.me link")
async def paypal_command(ctx):
    embed = discord.Embed(title="PayPal Payment", color=0x2E3135, description=f"\n➜[@Baraa Nimr](https://www.paypal.com/paypalme/saqarbaraa)")
    embed.set_thumbnail(url="https://cdn.discordapp.com/icons/1072407607479894036/76119c775b6db07df2e51cc21f40ad54.png?size=1024")
    embed.set_footer(text="discord.gg/boostworld")
    return await ctx.respond(embed=embed)
